"""
30_environmental_driver_rzsm.py

Purpose: Test root-zone soil moisture (RZSM, ERA5-Land layers 2+3,
7-100cm depth) as an additional driver, continuing the Stage 29 chain
(baseline + VPD + wind + solar radiation, which ended at neighbor
coefficient 0.0546).

RZSM is distinct from TerraClimate's surface soil moisture already
used in earlier stages - it specifically targets the depth range most
relevant to tree root water uptake in tropical forest.

Uses the CORRECTED bounding-box calculation (see project log).
...
Output: data/processed/rzsm_driver_results.csv
...
def main():
    ...
    print("RZSM range:", np.nanmin(rzsm_vals), "-", np.nanmax(rzsm_vals), "m^3/m^3")
    ...
    specs = [
        ...
        ("+ RZSM",
         "own_vod_t1 ~ own_vod_t + neighbor_vod_t + precip_anomaly + temp_anomaly + soil_anomaly + pdsi_anomaly + oni_t + vpd_anomaly + wind_anomaly + solar_anomaly + rzsm_anomaly"),
    ]
    ...
    for label, formula in specs:
        ...
        print(f"{label:42s}: neighbor coef={coef:.4f}  p={pval:.4f}")
        results.append((label, coef, pval))

    results_df = pd.DataFrame(results, columns=["model", "neighbor_coef", "neighbor_pval"])
    results_df.to_csv(os.path.join(OUT_DIR, "rzsm_driver_results.csv"), index=False)
    ...
    print(f"\n===== SUMMARY =====")
    print(f"Neighbor coefficient: {first_coef:.4f} -> {last_coef:.4f} ({pct_change:.1f}% total change)")
    rzsm_only_change = 100 * (results_df.iloc[1]["neighbor_coef"] - results_df.iloc[2]["neighbor_coef"]) / results_df.iloc[1]["neighbor_coef"]
    print(f"RZSM's own incremental contribution: {rzsm_only_change:.1f}%")
