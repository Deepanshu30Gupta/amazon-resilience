...
Output: data/processed/gnn_four_model_results_normalized.csv
        data/processed/learned_adjacency_weights_v2_normalized.csv (Model 3)
        data/processed/attention_weights_normalized.csv (Model 4)
"""
...
def main():
    ...
    results_df.to_csv(os.path.join(OUT_DIR, "gnn_four_model_results_normalized.csv"), index=False)
    ...
    pd.DataFrame(top_connections, columns=["patch_id", "learned_neighbor_id", "weight"]).to_csv(
        os.path.join(OUT_DIR, "learned_adjacency_weights_v2_normalized.csv"), index=False)
    ...
    pd.DataFrame(attn_records, columns=["patch_id", "neighbor_id", "attention_weight"]).to_csv(
        os.path.join(OUT_DIR, "attention_weights_normalized.csv"), index=False)

    print("\nSaved learned_adjacency_weights_v2_normalized.csv (Model 3) and attention_weights_normalized.csv (Model 4)")
