"""
34_gnn_data_prep_normalized.py

(REVISED from the original 34_gnn_data_prep_expanded.py: this version
adds feature STANDARDIZATION, fixing a real bug discovered while
building Stage 38 - raw feature scales varied enormously, e.g. solar
radiation anomaly ~10^7-10^8 vs VOD anomaly ~0.1, which meant large-
magnitude features completely dominated every neural network layer's
output in Stages 35-37, regardless of actual learned relevance. This
version standardizes every feature to comparable scale (z-score, using
TRAINING MONTHS ONLY for the mean/std, to avoid any leakage from
validation/test data) before building the sequence tensors.

Output is saved under a NEW filename (gnn_tensors_expanded_normalized.pt)
so the original gnn_tensors_expanded.pt and the Stages 35-38 results
built on it remain available as a pre-normalization diagnostic
comparison point - not deleted, but not to be used as final evidence.)

Purpose: Rebuild the GNN input tensors with the full ~15-variable
feature set, supporting multiple forecast horizons, with explicit,
documented handling of missing data, AND proper feature normalization.
...
def main():
    ...
    print("\nFeature ranges (post-imputation, BEFORE normalization):")
    for i, name in enumerate(all_feature_names):
        print(f"  {name:18s}: {full_tensor[:,:,i].min():.4f} to {full_tensor[:,:,i].max():.4f}")

    # ---- STANDARDIZATION (z-score), using TRAINING MONTHS ONLY =====
    # This is the critical fix: raw feature scales vary enormously (e.g.
    # solar radiation anomaly ~10^7-10^8 vs VOD anomaly ~0.1), which
    # without normalization would let large-magnitude features like solar
    # radiation completely dominate every neural network layer regardless
    # of actual learned relevance. Mean/std are computed using ONLY the
    # months that fall in the training period (before VAL_START), then
    # applied to train, validation, AND test data alike - using
    # validation/test data to compute normalization stats would leak
    # future information into the "training" process.
    print("\n===== Standardizing features (train-months-only mean/std) =====")
    train_month_mask = dates_sorted < pd.Timestamp(VAL_START)
    feature_mean = full_tensor[train_month_mask].mean(axis=(0, 1))  # (n_features,)
    feature_std = full_tensor[train_month_mask].std(axis=(0, 1))
    feature_std[feature_std < 1e-8] = 1.0  # avoid divide-by-zero for any constant feature

    print("Per-feature mean / std (from training months only):")
    for i, name in enumerate(all_feature_names):
        print(f"  {name:18s}: mean={feature_mean[i]:.4f}  std={feature_std[i]:.4f}")

    full_tensor = (full_tensor - feature_mean[np.newaxis, np.newaxis, :]) / feature_std[np.newaxis, np.newaxis, :]

    print("\nFeature ranges (AFTER normalization - should all be roughly comparable scale now):")
    for i, name in enumerate(all_feature_names):
        print(f"  {name:18s}: {full_tensor[:,:,i].min():.4f} to {full_tensor[:,:,i].max():.4f}")
    ...
    torch.save(tensors, os.path.join(OUT_DIR, "gnn_tensors_expanded_normalized.pt"))

    print("\n===== SAVED =====")
    print("IMPORTANT: saved as gnn_tensors_expanded_normalized.pt (a NEW file) - the original")
    print("gnn_tensors_expanded.pt from the first Stage 34 run is left untouched, so the")
    print("pre-normalization diagnostic results (Stages 35-38) remain available for comparison.")
    print("\nALSO IMPORTANT: since VOD (the prediction target) is feature index 0 and gets")
    print("normalized along with everything else, new RMSE values will be in STANDARDIZED")
    print("units, not the original raw VOD-anomaly units - they are NOT directly comparable")
    print("to the pre-normalization RMSE numbers. Compare relative rankings between models,")
    print("not absolute RMSE values, when comparing against the old results.")
    ...
    print(f"\nSaved to {OUT_DIR}/gnn_tensors_expanded_normalized.pt")
