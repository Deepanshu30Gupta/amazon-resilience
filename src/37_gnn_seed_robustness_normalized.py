"""
37_gnn_seed_robustness_normalized.py

NORMALIZED RERUN of 37_gnn_seed_robustness.py: identical model code and
procedure (same 5 seeds), but loads the feature-standardized tensors
(gnn_tensors_expanded_normalized.pt, from 34_gnn_data_prep_normalized.py)
instead of the pre-normalization ones, and writes
gnn_seed_robustness_results_normalized.csv. Corrected-pipeline counterpart
to 35_gnn_four_model_comparison_normalized.py.

Purpose: Test whether the GNN comparison is robust across random seeds,
on the corrected/normalized pipeline, or whether results could be an
artifact of one particular training run. This directly addresses the
strongest remaining criticism of the GNN comparison: "your dataset is
small, maybe the result depends on the seed."
...
Output: data/processed/gnn_seed_robustness_results_normalized.csv
"""
...
def main():
    ...
    results_df.to_csv(os.path.join(OUT_DIR, "gnn_seed_robustness_results_normalized.csv"), index=False)
